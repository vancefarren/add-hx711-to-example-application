...
static int hx711_attr_set_rate(const struct device *dev, const struct sensor_value *val)
{
	...
	if (cfg->rate.port == NULL) {
		LOG_ERR("Rate pin not configured for instance %d", cfg->instance);
		return -ENOTSUP;
	}
	switch (val->val1) {
			  ...
			  enum sensor_attribute attr, struct sensor_value *val)
{
	...
	switch ((enum hx711_attribute)attr) {
	...
	case SENSOR_ATTR_SLOPE:
		...
		LOG_DBG("Attribute SLOPE is %d.%d", data->slope.val1, data->slope.val2);
		return ret;
			  ...
			  enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_CALIBRATION:
		...
		LOG_DBG("Attribute CALIBRATION set to %d.%d",
			data->calibration_weight.val1, data->calibration_weight.val2);
		return ret;
	case SENSOR_ATTR_SLOPE:
		data->slope = *val;
		LOG_DBG("Attribute SLOPE set to %d.%d", data->slope.val1, data->slope.val2);
		return ret;
	case SENSOR_ATTR_GAIN:
			     ...
			     struct sensor_value *val)
{
	...
	switch ((enum hx711_channel)chan) {
	case SENSOR_CHAN_WEIGHT:
	{
		sensor_value_from_double(val, (sensor_value_to_double(&data->slope) *
...
static int hx711_init(const struct device *dev)
{
	...
	data->slope.val1 = data->sl[0];
	data->slope.val2 = data->sl[1] * 100000;
	LOG_DBG("Slope : %d.%06d", data->slope.val1, data->slope.val2);

	data->calibration_weight.val1 = data->cw[0];
	data->calibration_weight.val2 = data->cw[1] * 100000;
...
struct sensor_value avia_hx711_calibrate(const struct device *dev, uint8_t readings)
{
	...
	LOG_DBG("Calibration weight : %d.%06d",
		data->calibration_weight.val1, data->calibration_weight.val2);
	if (readings == 0) {
		readings = 1;
	}
	...
	LOG_DBG("Target set to : %d.%06d", data->calibration_weight.val1,
		data->calibration_weight.val2);
	double slope = sensor_value_to_double(
		&data->calibration_weight) / (double)(avg - data->offset);
...
#define HX711_INIT(index)							      \
	static struct hx711_data hx711_data_##index = {				      \
		.reading = 0,							      \
		.offset = DT_INST_PROP(index, offset),				      \
		.cw = DT_INST_PROP(index, calibration_weight),			      \
		.sl = DT_INST_PROP(index, slope),				      \
		.sample_fetch_timeout = DT_INST_PROP(index, sample_fetch_timeout_ms), \
		.gain = DT_INST_PROP(index, gain),				      \
		.rate = DT_INST_PROP(index, rate_hz),				      \
	};									      \
	...
	PM_DEVICE_DT_INST_DEFINE(index, hx711_pm_action);			      \
										      \
	DEVICE_DT_INST_DEFINE(index, hx711_init,				      \
			      PM_DEVICE_DT_INST_GET(index),			      \
			      &hx711_data_##index,				      \
			      &hx711_config_##index, POST_KERNEL,		      \
